...
const buildAll = async () => {
  ...
  packageJson.main = "maas-ui.umd.js";
  packageJson.module = "maas-ui.mjs";
  packageJson.types = "maas-ui.d.ts";
  fs.outputFile(
    path.resolve(baseOutDir, `package.json`),
    JSON.stringify(packageJson, null, 1)
  );
  ...
  // 生成配置DTS配置文件入口
  generateDTS(path.resolve(config.build.outDir, `maas-ui.mjs`));
